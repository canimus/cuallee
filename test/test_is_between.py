...
def test_between_number_with_pct(spark):
    check = Check(CheckLevel.WARNING, "CheckIsBetween")
    assert check.is_between("id", 0, 5, pct=0.5).validate(spark, spark.range(10)).first().status
...
def test_between_dates(spark):
    ...
    assert check.is_between("date", "2022-01-01", "2022-01-10").validate(spark, df).first().status
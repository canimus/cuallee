...
from cuallee.core.check import Check, CheckLevel, CheckStatus, ENGINES
...
class TestCheck:
    ...
    def test_validate_empty_check(self, check):
        with pytest.raises(AssertionError, match="Check is empty. Try adding some rules?"):
            check.validate(Mock())
    ...
    def test_validate_unsupported_engine(self, check):
        ...
        with pytest.raises(NotImplementedError, match="unittest.mock.Mock is not yet implemented in cuallee"):
    ...
    def test_validate_invalid_data_types(self, check):
        ...
        with patch('importlib.import_module', return_value=mock_engine):
            with pytest.raises(AssertionError, match="Invalid data types between rules and dataframe"):
                check.validate(mock_dataframe) 
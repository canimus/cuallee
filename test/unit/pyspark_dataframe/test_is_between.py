...
@pytest.mark.parametrize(
    ...
    [
        ["id", "zeros", 1, 0, "date", "date", tuple([date(2022, 9, 1), date(2022, 11, 1)])],
        ["zeros", "id", 22, 5, "timestamp", "timestamp", tuple([datetime(2022, 10, 22, 0, 0, 0), datetime(2022, 10, 22, 22, 0, 0)])]],
    ids=["date", "timestamp"],
)
def test_parameters_dates(spark, col_days, col_hours, start_day, start_hour, data_type, rule_column, rule_value):
    df = spark.range(10).withColumn('zeros', F.lit(0)).withColumn(rule_column, F.concat(F.lit("2022-10-"), F.col(col_days) + start_day, F.lit(" "), F.col(col_hours) + start_hour, F.lit(":0:0")).cast(data_type))
    check = Check(CheckLevel.WARNING, "pytest")
    check.is_between(rule_column, rule_value)
    rs = check.validate(df)
    assert rs.first().status == "PASS"
